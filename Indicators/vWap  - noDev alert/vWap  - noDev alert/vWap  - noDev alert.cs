...
namespace cAlgo.Indicators
{
    ...
    {
        [Output("VWAP", PlotType = PlotType.Points, Thickness = 2, Color = Colors.AliceBlue)]
        ...
        [Parameter("BG Bull Color", DefaultValue = "MediumSpringGreen", Group = "Color")]
        public string BGColorU_ { get; set; }

        [Parameter("BG Bear Color", DefaultValue = "IndianRed", Group = "Color")]
        public string BGColorD_ { get; set; }

        [Parameter("Outline intensity", DefaultValue = 185, MinValue = 0, MaxValue = 255, Group = "Color")]
        public int BGColorI_ { get; set; }
...
//       private int end_bar = 0;
        ...
        private bool SelectVoice(string Sim)
        {
            CmdArgs = "-Command \"Add-Type –AssemblyName System.Speech; (New-Object System.Speech.Synthesis.SpeechSynthesizer).Speech.SelectVoice('" + Sim + "');\"";
            {
        ...
        {
            sstr = sstr.Split('.')[0]; // remove .trash
...
//          general replacements
            ...
            sstr = sstr.Replace("US2000", "Russel");
            ...
            sstr = sstr.Replace("DE40", "dax");
            sstr = sstr.Replace("GER40", "dax");

            sstr = sstr.Replace("FRA40", "CAC");

            sstr = sstr.Replace("NAS100", "Nasdaq");
            sstr = sstr.Replace("US100", "Nasdaq");
            sstr = sstr.Replace("USTEC", "Nasdaq");
            ...
            sstr = sstr.Replace("DOGEUSD", "Doge");
            sstr = sstr.Replace("BTCUSD", "Bitcoin");
            sstr = sstr.Replace("ETHUSD", "Ethereum");

//          Currency parts
            sstr = sstr.Replace("CHF", " swiss ");
        ...
        protected override void Initialize()
        {
            ...
            SelectVoice("Microsoft Richard Desktop");
        ...
        public override void Calculate(int index)
        {
            ...
            if (IsLastBar && PaintChart)
            {
                ...
                double cl1 = (((atr.Result[index] / Symbol.PipSize)) * 1.02);
                ...
                // make sure its never larger than FF
                if (alpha > 255)
                {
                    alpha = 255;
                }
...
//                    Chart.ColorSettings.BackgroundColor = Color.FromArgb(alpha, BGColorU_);
                    Chart.ColorSettings.BullFillColor = Color.FromArgb(alpha, BGColorU_);
                    Chart.ColorSettings.BearFillColor = Color.FromArgb(alpha, BGColorU_);
                    Chart.ColorSettings.BullOutlineColor = Color.FromArgb(Convert.ToInt32(BGColorI_), BGColorU_);
                    Chart.ColorSettings.BearOutlineColor = Color.FromArgb(Convert.ToInt32(BGColorI_), BGColorU_);
                }
                else
                {
//                    Chart.ColorSettings.BackgroundColor = Color.FromArgb(alpha, BGColorD_);
                    Chart.ColorSettings.BullFillColor = Color.FromArgb(alpha, BGColorD_);
                    Chart.ColorSettings.BearFillColor = Color.FromArgb(alpha, BGColorD_);
                    Chart.ColorSettings.BullOutlineColor = Color.FromArgb(Convert.ToInt32(BGColorI_), BGColorD_);
                    Chart.ColorSettings.BearOutlineColor = Color.FromArgb(Convert.ToInt32(BGColorI_), BGColorD_);
...
//                    Chart.ColorSettings.BearOutlineColor = Color.FromArgb(255, Chart.ColorSettings.BackgroundColor.ToArgb() ^ 0xffffff);
                    //                       Chart.ColorSettings.BullOutlineColor = Color.Orange;
                    //                       Chart.ColorSettings.BearOutlineColor = Color.Orange;

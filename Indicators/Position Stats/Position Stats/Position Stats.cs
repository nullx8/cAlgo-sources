...
namespace cAlgo
{
    ...
    public class PositionStats : Indicator
    {
        ...
        public void update()
        {
            ...
            {
                if (showLabels)
                    s.Append("Buy positions: ");
                s.AppendFormat("{0:N0}", buy_count);
                s.AppendLine();

                if (showLabels)
                    s.Append("Sell positions: ");
...
//            ChartObjects.DrawText("showInfo", s.ToString(), pos);
            if (Account.MarginLevel.Value < 2500)
            {
                ChartObjects.DrawText(Symbol.Code + "showInfo", s.ToString(), pos, Colors.Red);
            }
            else
            {
                ChartObjects.DrawText(Symbol.Code + "showInfo", s.ToString(), pos, Colors.Gray);
            }
        }

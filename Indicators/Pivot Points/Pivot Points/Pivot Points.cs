...
//        [Parameter("Pivot Color", DefaultValue = "LightSteelBlue")]
...
//        public string ResistanceColor { get; set; }
        ...
        private void CalculatePivots(DateTime startTime, int startIndex, DateTime startTimeOfNextPeriod, int index)
        {
            ...
            ChartObjects.DrawLine("pivot " + startIndex, pivotStartTime, pivot, pivotEndTime, pivot, Colors.Gray);
            ChartObjects.DrawLine("r1 " + startIndex, pivotStartTime, r1, pivotEndTime, r1, Colors.DarkSlateGray);
            ChartObjects.DrawLine("r2 " + startIndex, pivotStartTime, r2, pivotEndTime, r2, Colors.DarkSlateGray);
            ChartObjects.DrawLine("r3 " + startIndex, pivotStartTime, r3, pivotEndTime, r3, Colors.DarkSlateGray);
            ChartObjects.DrawLine("s1 " + startIndex, pivotStartTime, s1, pivotEndTime, s1, Colors.Maroon);
            ChartObjects.DrawLine("s2 " + startIndex, pivotStartTime, s2, pivotEndTime, s2, Colors.Maroon);
            ChartObjects.DrawLine("s3 " + startIndex, pivotStartTime, s3, pivotEndTime, s3, Colors.Maroon);

            if (!ShowLabels)
                return;

            ChartObjects.DrawText("Lpivot " + startIndex, "P (" + tf + ")", index, pivot, vAlignment, hAlignment, Colors.Azure);
            ChartObjects.DrawText("Lr1 " + startIndex, "R1 (" + tf + ")", index, r1, vAlignment, hAlignment, Colors.DarkSlateGray);
            ChartObjects.DrawText("Lr2 " + startIndex, "R2 (" + tf + ")", index, r2, vAlignment, hAlignment, Colors.DarkSlateGray);
            ChartObjects.DrawText("Lr3 " + startIndex, "R3 (" + tf + ")", index, r3, vAlignment, hAlignment, Colors.DarkSlateGray);
            ChartObjects.DrawText("Ls1 " + startIndex, "S1 (" + tf + ")", index, s1, vAlignment, hAlignment, Colors.Maroon);
            ChartObjects.DrawText("Ls2 " + startIndex, "S2 (" + tf + ")", index, s2, vAlignment, hAlignment, Colors.Maroon);
            ChartObjects.DrawText("Ls3 " + startIndex, "S3 (" + tf + ")", index, s3, vAlignment, hAlignment, Colors.Maroon);

...
namespace cAlgo.Indicators
{
    ...
    [Levels()]
    public class BreakOutRSI1_Indicator : Indicator
    {
        Mq4Double Mq4Init()
        {
            return 0;
        ...
        Mq4Double Mq4Start()
        {
            ...
            double SDA = ((_standardDeviation.Result[_currentIndex] / 100) * 80);
            RSI = iRSI(NULL, 0, 10, PRICE_OPEN, 0);

            Top = iHigh(NULL, 0, iHighest(NULL, 0, MODE_HIGH, 16, 1));
            Bottom = iLow(NULL, 0, iLowest(NULL, 0, MODE_LOW, 16, 1));
            ...
            ObjectSet("B-Range", OBJPROP_COLOR, DarkSlateBlue);
            ...
            ObjectSet("B-Range2", OBJPROP_COLOR, DarkSlateBlue);
            ...
            ObjectSet("A-Range", OBJPROP_COLOR, DarkGray);
            ...
            if (Close[0] >= AvgHighs && Close[0] < Top)
            {
                Trend = 1;
            }
            if (Close[0] < AvgHighs && Close[0] > AvgLows)
            {
                Trend = 0;
            }
            if (Close[0] <= AvgLows && Close[0] > Bottom)
            {
                Trend = -1;
            }
            if (Close[0] <= Bottom)
            ...
            ObjectSetText("RSI", "RSI: " + DoubleToStr(RSI, 1), 8, "Arial Black", White);
            ObjectSet("RSI", OBJPROP_CORNER, 0);
            ObjectSet("RSI", OBJPROP_XDISTANCE, 4);
            ObjectSet("RSI", OBJPROP_YDISTANCE, 34);


                        /*
ObjectCreate("BrLong", OBJ_LABEL, 0, 0, 0);
            ObjectSetText("BrLong", "SL: " + DoubleToStr(((Top - Bottom) * 10000), 1), 8, "Arial Black", Red);
            ObjectSet("BrLong", OBJPROP_CORNER, 0);
            ObjectSet("BrLong", OBJPROP_XDISTANCE, 4);
            ObjectSet("BrLong", OBJPROP_YDISTANCE, 53);
*/

//double SDA = _standardDeviation.Result[0];
//            ChartObjects.DrawText("sda", "SD: " + SDA, 0);

//ObjectCreate("BrShort", OBJ_LABEL, 0, 0, 0);
            //          ObjectSetText("BrShort", "SD: " + DoubleToStr(SDA, Dig), 8, "Arial Black", Red);
            //          ObjectSet("BrShort", OBJPROP_CORNER, 0);
            //          ObjectSet("BrShort", OBJPROP_XDISTANCE, 4);
            //          ObjectSet("BrShort", OBJPROP_YDISTANCE, 53);
...
//{
        ...
        class Mq4OutputDataSeries : IMq4DoubleArray
        {
            ...
            private readonly BreakOutRSI1_Indicator _indicator;

            public Mq4OutputDataSeries(BreakOutRSI1_Indicator indicator, IndicatorDataSeries outputDataSeries, ChartObjects chartObjects, int style, int bufferIndex, Func<IndicatorDataSeries> dataSeriesFactory, int lineWidth, Colors? color = null)
            {
            ...
            public override void Draw()
            {
                ...
                switch (ArrowCode)
                {
                    ...
                    case SYMBOL_LEFTPRICE:
                        ...
                        break;
                    default:
                        arrowString = BreakOutRSI1_Indicator.GetArrowByCode(ArrowCode);

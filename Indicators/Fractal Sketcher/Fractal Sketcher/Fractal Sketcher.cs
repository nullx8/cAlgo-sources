...
namespace cAlgo.Indicators
{
    ...
    public class FractalSketcher : Indicator
    {
        [Parameter(DefaultValue = 48, MinValue = 3)]
        public int Calc_Max_Bars_For_TL { get; set; }

        [Parameter(DefaultValue = 5, MinValue = 3)]
        public int Period { get; set; }

        [Output("Up Fractal", Color = Colors.WhiteSmoke, PlotType = PlotType.Points, Thickness = 5)]
        public IndicatorDataSeries UpFractal { get; set; }

        [Output("Down Fractal", Color = Colors.WhiteSmoke, PlotType = PlotType.Points, Thickness = 5)]
...
//      private RelativeStrengthIndex _rsi;
        ...
        {
            UpFractal = CreateDataSeries();
            DownFractal = CreateDataSeries();
        ...
        {
//           ChartObjects.DrawText("show", logD + "\n" + logU, StaticPosition.BottomLeft);
            index_high_1 = 0;
            index_high_2 = 0;
            index_low_1 = 0;
            index_low_2 = 0;
        ...
        private void DrawTrendLine(string lineName, int startIndex, int endIndex, int index1, double value1, int index2, double value2, bool color_up_trend)
        {
            ...
            {
                Calc_Max_Bars_For_TL = (index1 - index2) * 12;
                ChartObjects.DrawLine(lineName, index2, value2, endIndex, endValue, Colors.DarkSlateGray, 2, LineStyle.DotsRare);
            }
            else
            {
                logU = logU + " " + Calc_Max_Bars_For_TL;
                Calc_Max_Bars_For_TL = (index1 - index2) * 12;
                ChartObjects.DrawLine(lineName, index2, value2, endIndex, endValue, Colors.DarkSlateGray, 2, LineStyle.DotsRare);
                logD = " " + Calc_Max_Bars_For_TL;
//                logD = Calc_Max_Bars_For_TL;
            }
        }

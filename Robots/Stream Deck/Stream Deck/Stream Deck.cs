            ...
            if ((int.TryParse(message.Split(',')[0], out keyId)) && (allowExecute))
            {
                ...
                else
                {
                    ...
                    if (command == "RemoveTPSL")
                    {
                        ...
                        {
                            ModifyPositionAsync(pos, null, null,false);
                        }
...
//                var pNLt = "";
                ...
                if (Account.MarginLevel.HasValue)
                {
                    if (FreeeMargin > 2000)
                    {
                        FreeMargint = ""+ Math.Round(Account.Equity / AccountBalanceAtTime(Time.Date) * 100 - 100, 2);
                    }
                    else
                    {
                        FreeMargint = " m " + Math.Round(Account.MarginLevel.Value, 0) + "%";
                    }
                }
                else
                {
                    FreeMargint = "" + Math.Round(pNL,2);
                }
...
*/
                {
                    sendImage(defaultButton, "  Idle ...\n  " + Math.Round(Account.Equity / AccountBalanceAtTime(Time.Date) * 100 - 100, 3) + "%", "orange", 23);

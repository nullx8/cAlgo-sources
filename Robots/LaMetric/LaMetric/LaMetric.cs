    ...
    public class LaMetric : Robot
    {
        ...
        protected void UpdateLaMetric()
        {
            ...
            if (ShowMoney) {
                ...
                if (WifeMode) {
                    todayProfit = Account.Balance - (DayStart+unrealizedProfit);
//                    todayProfit = 0;
                }
//                todayProfit = 0;
            }
...
//          frames.Add(GetValueFrame(todayProfit, true));
            ...
            if ((todayProfit ==0)&&(Positions.Count==0)) {
                frames.Add(GetTextFrame(Icon.Ctrader, "Ctrader"));
            }
            else {
                if (ShowMoney) {
                    if (todayProfit >0) {
                        frames.Add(GetTextFrame(Icon.Ctrader, "+$" + Math.Round(todayProfit, Math.Abs(todayProfit) > 100 ? 0 : 3) ));
                    } else {
                        frames.Add(GetTextFrame(Icon.Ctrader, "($" + Math.Round(Math.Abs(todayProfit), Math.Abs(todayProfit) > 1000 ? 0 : 3) + ")"));
                    }
                }
                else {
                    if (todayProfit >0) {
                        frames.Add(GetTextFrame(Icon.Ctrader, "+" + Math.Round(todayProfit, Math.Abs(todayProfit) > 1 ? 2 : 3) + "%"));
                    } else {
                        frames.Add(GetTextFrame(Icon.Ctrader, Math.Round(todayProfit, Math.Abs(todayProfit) > 1 ? 2 : 3) + "%"));
                    }
                }
